// Same Driver API FFI approach as matmul_gpu.rs (no CUDA crate in std, this
// repo keeps every Rust benchmark a dependency-free single `rustc -O`
// compile), but launches the kernel N times in one warm process instead of
// once, to see whether Mojo's ~0.3-0.4s GPU overhead is a one-time MAX
// startup cost (which N repeated launches should amortize away) or a
// per-launch cost (which it wouldn't).
use std::env;
...
fn ptx_path() -> PathBuf {
    ...
    path.set_file_name("matmul_gpu_warm.ptx");
    path
}

fn main() {
    let iterations: i32 = env::args().nth(1).map(|s| s.parse().unwrap()).unwrap_or(500);
    let n = N;
    let nu = n as usize;
    ...
    unsafe {
        ...
        let last_offset = ((nu * nu - 1) * std::mem::size_of::<f64>()) as u64;
        let mut corner0 = 0.0f64;
        let mut corner_n = 0.0f64;
        for it in 0..iterations {
            ...
            check(cuMemcpyDtoH_v2(&mut corner0 as *mut f64 as *mut c_void, d_c, 8), "cuMemcpyDtoH_v2(corner0)");
            check(cuMemcpyDtoH_v2(&mut corner_n as *mut f64 as *mut c_void, d_c + last_offset, 8), "cuMemcpyDtoH_v2(cornerN)");
            if corner0 != expected {
                panic!("self-check failed at iteration {}: c[0][0] mismatch", it);
            }
        }
